    ...
    public class InventarioController : ControllerBase
    {
            ...
            [FromQuery] bool soloConStock = false)
        {
            ...
            var cmd = new NpgsqlCommand($@"
                ...
                LIMIT @Size OFFSET @Offset", conexion);
        ...
        private static void AgregarParametrosFiltro(NpgsqlCommand cmd, string? q, int? categoriaId, bool soloConStock)
        {
        ...
        public IActionResult Eliminar(int id)
        {
            ...
            var cmd = new NpgsqlCommand("DELETE FROM Inventario WHERE Id = @Id", conexion);
            cmd.Parameters.AddWithValue("@Id", id);
        ...
        private int? ResolverCategoriaId(NpgsqlConnection conexion, Producto p)
        {
            // Camino 1: id explícito (preferido)
            if (p.CategoriaId.HasValue)
            {
                var cmd = new NpgsqlCommand("SELECT 1 FROM Categorias WHERE Id = @Id", conexion);
            ...
            {
                var cmd = new NpgsqlCommand("SELECT Id FROM Categorias WHERE Nombre = @Nombre", conexion);
        ...
        private void AgregarParametros(NpgsqlCommand cmd, Producto p, int? categoriaId)
        {
